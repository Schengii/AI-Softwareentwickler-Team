...
class TestCheckFrontendBuild(unittest.TestCase):
    ...
    def test_missing_node_modules_is_reported_as_failed_build(self):
        """Team-Optimierung (echter Fund): EnvironmentMixin.ensure_environment() führt für JEDES
        Frontend-Projekt mit "build"-Skript bereits `npm install`/`npm ci` aus, BEVOR dieser Check
        läuft - fehlende node_modules trotz verfügbarem `npm` bedeuten deshalb fast immer eine
        fehlgeschlagene Installation, nicht "nicht prüfbar". Das alte stille
        `attempted=False, passed=True` ließ ein kaputtes Frontend als "geprüft und bestanden"
        durchgehen; jetzt gilt das als fehlgeschlagener Build (attempted=True, passed=False), der
        an den Fix-Loop zurückgemeldet wird."""
