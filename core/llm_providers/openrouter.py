"""
core/llm_providers/openrouter.py – OpenRouterClient (P6-5, ROADMAP_TEMP.md, aus
...
class OpenRouterClient:
    ...
    async def generate_with_usage(
        ...
        if not _lf.OPENROUTER_API_KEY or token_guard.is_model_exhausted(f"openrouter:{self.model_name}"):
            ...
            return await _lf._cross_provider_failover_with_usage("openrouter", prompt, system_prompt)

        headers = {
            "Authorization": f"Bearer {_lf.OPENROUTER_API_KEY}",
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200 and "choices" in data:
                    ...
                    if response.status_code in (401, 402, 429) or "balance" in err_msg.lower() or "limit" in err_msg.lower():
                        token_guard.mark_model_exhausted(f"openrouter:{self.model_name}", f"OpenRouter: {err_msg}", cooldown_seconds=_lf._exhaustion_cooldown_seconds(err_msg))
                    if not _allow_self_fallback:
                        raise RuntimeError(f"OpenRouter-Fehler innerhalb einer Fallback-Kette: {err_msg}")
                    return await _lf._cross_provider_failover_with_usage("openrouter", prompt, system_prompt)

        except Exception as e:
            token_guard.mark_model_exhausted(f"openrouter:{self.model_name}", str(e), cooldown_seconds=_lf._exhaustion_cooldown_seconds(str(e)))
            if not _allow_self_fallback:
                raise
            return await _lf._cross_provider_failover_with_usage("openrouter", prompt, system_prompt)
    ...
    async def generate_with_tools(
        ...
        if not _lf.OPENROUTER_API_KEY or token_guard.is_model_exhausted(f"openrouter:{self.model_name}"):
            if not _allow_self_fallback:
                raise RuntimeError("OpenRouter innerhalb einer Fallback-Kette nicht verfügbar (kein Key/erschöpft).")
            return await _lf._cross_provider_failover_with_tools("openrouter", messages, system_prompt, tools)
