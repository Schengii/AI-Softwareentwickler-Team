"""
tests/test_framework_release_cli_command.py – Testet den CLI-Befehl `/release`

interface/cli.py._create_release_with_confirmation() ruft core/framework_release.py auf, um
den nächsten SemVer-Bump vorzuschlagen, und erstellt bei Bestätigung einen echten Tag +
GitHub-Release. github_agent hier gemockt (core/framework_release.py selbst ist bereits real
gegen ein Git-Repo getestet, siehe test_framework_release.py) - analog zu
...
class TestReleaseCliCommand(unittest.TestCase):
    ...
    @patch("interface.cli.framework_release.create_release")
    @patch("interface.cli.framework_release.get_commits_since")
    @patch("interface.cli.framework_release.get_latest_tag")
    ...
    def test_missing_gh_cli_blocks_release_before_any_lookup(self):
        ...
        with patch("interface.cli.framework_release.get_latest_tag") as mock_latest:
            asyncio.run(self.cli._create_release_with_confirmation())
